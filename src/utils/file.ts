...
export const initFolder = () => {
  ;[UPLOAD_IMAGE_TEMP_DIR, UPLOAD_VIDEO_TEMP_DIR].forEach((dir) => {
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir, {
        recursive: true // mục đích tạo folder nested không bị lỗi vd trên nhữ nestes /images
      })
    }
  })
}
...
export const handleUploadSingleImage = async (req: Request) => {
  ...
  const form = formidable({
    ...
    maxFields: 1,
    keepExtensions: true,
    maxFieldsSize: 300 * 1024, // 300KB --- 3 * 1024 * 1024 = 300MB
...
/**
 * Cách sử lí khi upload video và encode
 * có 2 giai đoạn
...
export const handleUploadVideo = async (req: Request) => {
  ...
  const form = formidable({
    ...
    maxFields: 1,
    // keepExtensions: true, // lấy đuôi .mp4 ,...
